...
function getNextFriday(date) {
  ...
  let dayGap = (5 - currentDay + 7) % 7;
  if (dayGap === 0) dayGap += 7;

  return new Date(copyDate.setDate(copyDate.getDate() + dayGap));
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const different = new Date(dateEnd).getTime() - new Date(dateStart).getTime();
...
function formatDate(date) {
  ...
  return `${Number(month) + 1}/${day}/${year}, ${hours === 12 ? 12 : hours % 12}:${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')} ${hours > 11 ? 'P' : 'A'}M`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  let weekendsCount = 0;
  for (let i = 1; i <= daysCountInMonth; i += 1) {
...
function getWeekNumberByDate(date) {
  ...
  const timeDiff = targetDate.getTime() - firstDayOfYear.getTime();
  const daysSinceFirstDay = Math.floor(timeDiff / (24 * 60 * 60 * 1000)) + 1;
...
function getQuarter(date) {
  const currentDate = new Date(date);
  const firstQuarterOfYear = new Date(currentDate.getFullYear(), 2, 31);
  const secondQuarterOfYear = new Date(currentDate.getFullYear(), 5, 30);
  const thirdQuarterOfYear = new Date(currentDate.getFullYear(), 9, 31);
  if (currentDate.getTime() > thirdQuarterOfYear.getTime()) {
    return 4;
  }
  if (currentDate.getTime() > secondQuarterOfYear.getTime()) {
    return 3;
  }
  if (currentDate.getTime() > firstQuarterOfYear.getTime()) {
    return 2;
  }
  return 1;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate.getTime() <= endDate.getTime()) {
    for (let a = 0; a < countWorkDays; a += 1) {
      if (currentDate > endDate) break;
      result.push(dateFormatter(currentDate));
